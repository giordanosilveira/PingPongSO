...
/**
 * @brief Acorda as tarefas que estão dormindo
 * 
 */
void awake_tasks(){
    ...
    // Percorre todas as tarefas da lista suspensas e as acorda se for o tempo correto
...
void task_resume(task_t *task, task_t **queue){
    ...
    if (queue_remove((queue_t **)(queue), (queue_t*)(task)) < 0){
        // queue_print("fila do task remove ", (queue_t *)t)
        fprintf(stderr, "task_resume(): "); 
...
void task_sleep(int t) {
    ...
    // Verifica se a tarefa atual não é nula
    if (! current_task) {
...
int mqueue_create(mqueue_t *queue, int max, int size) {
    ...
    queue->data_buffer = malloc(size*max);
...
int mqueue_send (mqueue_t *queue, void *msg) {
    ...
    queue->last_position++;
    if (queue->last_position == 5) {
        queue->last_position = 0;
    }
    queue->n_msg++;
    memcpy(queue->data_buffer + queue->last_position*queue->size_msg, msg, queue->size_msg);
...
int mqueue_recv(mqueue_t *queue, void *msg) {
    ...
    queue->last_item++;
    if (queue->last_item == 5)
        queue->last_item = 0;
    
    memcpy(msg, queue->data_buffer + queue->last_item*queue->size_msg, queue->size_msg);
    queue->n_msg--;
